...
RSpec.describe 'Application plans API', type: :integration do
  ...
  before(:each) do
    @plan = client.create_application_plan(service_id, name: name) # placeholder until we have direct access to DB
  end

  after(:each) do
    if @plan != nil
      begin
        client.delete_application_plan(service_id, @plan['id']) # placeholder until we have direct access to DB
  ...
  it 'delete application plan' do
    ...
    expect(client.list_service_application_plans(service_id).any? { |plan| plan['id'] == @plan['id'] }).to be(false)
  end
